...
class Car(models.Model):
    ...
    capacity = models.IntegerField(default=4)  # Number of passengers
    fuel_type = models.CharField(max_length=50)  # Petrol, Diesel, Electric, etc.
...
class Trip(models.Model):
    ...
    # References
    booking = models.OneToOneField('bookings.Booking', on_delete=models.CASCADE, related_name='trip', null=True, blank=True)
    ...
    start_fuel_level = models.DecimalField(max_digits=5, decimal_places=2, null=True, blank=True)  # in liters
    ...
    def __str__(self):
        return f"Trip: {self.start_location} → {self.end_location} ({self.start_time})"
...
class Document(models.Model):
    ...
    verified_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, limit_choices_to={'role': 'admin'}, related_name='verified_documents')
...
class ReviewRating(models.Model):
    ...
    def __str__(self):
        return f"{self.get_review_type_display()} Review - {self.rating} stars"
...
class UsedCarInquiry(models.Model):
    ...
    # Car info
    car = models.ForeignKey(Car, on_delete=models.CASCADE, related_name='inquiries', limit_choices_to={'car_type': 'used'})
...
class Complaint(models.Model):
    ...
    # Assignment
    assigned_to = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, limit_choices_to={'role__in': ['admin', 'manager']}, related_name='assigned_complaints')
...
class Payment(models.Model):
    ...
    # Gateway
    gateway_name = models.CharField(max_length=50)  # razorpay, stripe, etc.
...
class Invoice(models.Model):
    ...
    # Reference
    booking = models.ForeignKey('bookings.Booking', on_delete=models.CASCADE, related_name='invoices', null=True, blank=True)
...
class Refund(models.Model):
    ...
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, limit_choices_to={'role': 'admin'}, related_name='approved_refunds')
    ...
    # Additional
    rejection_reason = models.TextField(null=True, blank=True)
    refund_method = models.CharField(max_length=50, null=True, blank=True)  # original, wallet, bank, etc.
